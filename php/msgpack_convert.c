    ...
    if (*properties != NULL)
    {
        ...
        for (;; zend_hash_move_forward_ex(*properties, prop_pos))
        {
                    ...
                    &prop_key_index, 0, prop_pos) == HASH_KEY_IS_STRING)
            {
                ...
                {
                    if (zend_hash_find(ht, prop_key, prop_key_len, (void **)&data) == SUCCESS) {
                        switch (Z_TYPE_PP(data)) {
                            case IS_ARRAY:
                            {
                                HashTable *dataht;
                                dataht = HASH_OF(val);
                                if (zend_hash_index_find(dataht, prop_key_index, (void **)dataval) != SUCCESS) {
                                    MSGPACK_WARNING(
...
int msgpack_convert_array(zval *return_value, zval *tpl, zval **value)
{
    ...
    if (Z_TYPE_P(tpl) == IS_ARRAY) {
	    ...
	    htval = HASH_OF(*value);
	    if (! htval) {
		//zend_print_zval_r(*value, 1);
		//zend_print_zval_r(tpl, 1);
		MSGPACK_WARNING(
		...
		return FAILURE;
			...
			if (convert_function) {
			    //zend_print_zval_r(*data, 1);
			    //zend_print_zval_r(aryval, 1);
			    if (convert_function(rv, *data, &aryval) != SUCCESS)
			    {
				//zend_print_zval_r(rv, 1);
				zval_ptr_dtor(&aryval);
				MSGPACK_WARNING(
				    "[msgpack] (%s) convert failure in HASH_KEY_IS_LONG in indexed array",
				    __FUNCTION__);
				return FAILURE;
			    }
			    //zend_print_zval_r(rv, 1);
		...
		for (;; zend_hash_move_forward_ex(ht, &pos))
		{
			...
			ALLOC_INIT_ZVAL(rv);
                        ...
                        if (convert_function(rv, *data, &aryval) != SUCCESS)
                        {
                            zval_ptr_dtor(&aryval);
                            MSGPACK_WARNING(
                                "[msgpack] (%s) convert failure in convert_object",
                                __FUNCTION__);
                            return FAILURE;
                        }

                        zend_symtable_update(ret, key, key_len, &rv, sizeof(rv), NULL);
                        continue;
                    }

		    {
			//zend_print_zval_r(return_value, 1);
			//zend_print_zval_r(*value, 1);
			//zend_print_zval_r(aryval, 1);
			zend_symtable_update(ret, key, key_len, &aryval, sizeof(aryval), NULL);
		    }
                }
            }
